...
STATUS_TOO_MANY_DUCK = Status.TOO_MANY_DUCK
STATUS_NO_DUCK = Status.NO_DUCK


class Termination(enum.Enum):
    """Enum with reasons for a game to be over."""
    KING_CAPTURED = enum.auto()
    """See :func:`chess.Board.is_king_captured`."""
    STALEMATE = enum.auto()
...
class BaseBoard:
    ...
    def duckmoves(self) -> SquareSet:
        """
        Get all empty square the duck can goto.
        """
        for i in range(1,64):
            if self.piece_type_at(i) == None:
                ducksquare: list
                ducksquare.append(i)
        return self.ducksquare

    def duckmoves_mask(self, square: Square) -> Bitboard:
        return self.duckmoves(square)
...
class _BoardState(Generic[BoardT]):
    ...
    def __init__(self, board: BoardT) -> None:
        ...
        self.duckmove_clock = board.duckmove_clock
    ...
    def restore(self, board: BoardT) -> None:
        ...
        board.duckmove_clock = self.duckmove_clock
...
class Board(BaseBoard):
    ...
    duckmove_clock: bool
    """A varibal which tell if it is a duckmove now"""
    ...
    def genetate_pseudo_legal_duck_moves(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL) -> Iterator[Move]:
        if (from_mask & self.ducks) & (to_mask & self.duckmoves_mask):
            yield Move(from_mask, to_mask)
        else:
            return
    ...
    def outcome(self, *, claim_draw: bool = False) -> Optional[Outcome]:
        ...
        if self.is_king_captured():
            return Outcome(Termination.KING_CAPTURED, self.turn)
